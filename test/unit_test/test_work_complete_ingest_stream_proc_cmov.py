...
event_file = f"{CURRENT_DIR}/test_data_files/request/work_complete_ingest_stream_proc_cmov.json"
exp_response_file = f"{CURRENT_DIR}/test_data_files/expected_response/work_complete_ingest_stream_proc_cmov.json"


# Assert
@pytest.mark.parametrize(
    "setup_and_teardown, get_event_data, get_exp_response_data",
    [((False, "clean_up", input_file), event_file, exp_response_file)],
    indirect=True,
)
def test_work_complete_ingest_stream_proc_cmov(
    ...
    if dynamodb_response["Items"][0].get("work_order_complete"):
        assert True
    else:
        assert False
